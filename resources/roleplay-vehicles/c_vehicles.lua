...
local function displayVehicleOptions(x, y, cVehicle)
	...
	addEventHandler("onClientGUIClick", vehoptions.button[1],
		function()
			destroyElement(vehoptions.window[1])
			showCursor(false)
			triggerEvent(":_openVehicleInventory_:", localPlayer, vehicle)
		end, false
	)
...
local function checkForALTForDescription(button, press)
	if button == "lalt" then
    	if press then
...
addEventHandler("onClientKey", root, checkForALTForDescription)