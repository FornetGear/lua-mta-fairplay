...
local totalStreams = 20 --Update this with the number of streams
local theTimer
local streams = { }
...
function setVolume(commandname, val)
	if tonumber(val) then
		...
		if (tonumber(val) >= 0 and tonumber(val) <= 100) then
			triggerServerEvent(":_SyncRadioVolume_:", getLocalPlayer(), tonumber(val))
...
function showStation()
	...
	if (theVehicle) then
		local streamID = getElementData(theVehicle, "roleplay:vehicles.radio.station")
		local streamTitle
...
function saveRadio(station)	
	...
	if exports['roleplay-scoreboard']:isScoreboardEnabled() then
		cancelEvent()
		return
	end
	...
	if (vehicle) then
		if getVehicleOccupant(vehicle) == getLocalPlayer() or getVehicleOccupant(vehicle, 1) == getLocalPlayer() then
			...
			triggerServerEvent(":_SyncRadioStation_:", getLocalPlayer(), radio)
		end
		cancelEvent()
	end
end
...
addEventHandler("onClientPlayerVehicleEnter", getLocalPlayer(),
	function(theVehicle)
		stopStupidRadio()
		radio = getElementData(theVehicle, "roleplay:vehicles.radio.station") or 0
...
addEventHandler ( "onClientElementDataChange", getRootElement(),
	function ( dataName )
		if getElementType ( source ) == "vehicle" and dataName == "roleplay:vehicles.radio.station" then
			local newStation =  getElementData(source, "roleplay:vehicles.radio.station") or 0
		...
		elseif getElementType(source) == "vehicle" and dataName == "roleplay:vehicles.winstate" then
			if (isElementStreamedIn (source)) then
				if (soundElementsOutside[source]) then
					updateLoudness(source)
				end
			end
		elseif getElementType(source) == "vehicle" and dataName == "roleplay:vehicles.radio.station" then
			if (isElementStreamedIn (source)) then
				if (soundElementsOutside[source]) then
					updateLoudness(source)
				end
...
function spawnSound(theVehicle)
    ...
    if getElementType( theVehicle ) == "vehicle" then
		...
		if enableStreams == 1 then
			local radioStation = getElementData(theVehicle, "roleplay:vehicles.radio.station") or 0
...
function updateLoudness(theVehicle)
	if (soundElementsOutside[theVehicle]) then
		local windowState = getElementData(theVehicle, "roleplay:vehicles.winstate") or 1
		local carVolume = getElementData(theVehicle, "roleplay:vehicles.radio.volume") or 60
